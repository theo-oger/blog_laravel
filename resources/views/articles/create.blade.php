<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <form method="post" action="{{ route('articles.store') }}" class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                @csrf
                
                <div class="p-8 border-b border-gray-100">
                    <label for="title" class="block text-sm font-medium text-gray-700 mb-2">Titre de l'article</label>
                    <input type="text" name="title" id="title" placeholder="Saisissez un titre accrocheur..." class="w-full text-lg rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-colors">
                </div>
                ...
                <div class="p-8 border-b border-gray-100 bg-gray-50/30">
                    ...
                    <textarea rows="15" name="content" id="content" placeholder="Rédigez votre article ici..." class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-colors resize-y font-sans"></textarea>
                </div>
                            ...
                            @foreach ($categories as $category)
                                <label class="relative flex items-start p-4 cursor-pointer rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
                                    <div class="flex items-center h-5">
                                        <input type="checkbox" name="categories[]" id="category-{{ $category->id }}" value="{{ $category->id }}" class="h-4 w-4 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500">
                                    </div>
                ...
                <div class="p-8 bg-gray-50 flex items-center justify-between">
                    <div class="flex items-center">
                        <input type="checkbox" name="draft" id="draft" class="h-4 w-4 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500">
